...
*/
  ...
  if(count == 0)
  {
    ...
    memcpy(&tx_sync_msg[24], &default_txts, 8);
  ...
  else
  {
    ...
    // Fill the actual time stamp to the transmit buffer
    memcpy(&tx_sync_msg[24], &txstamp[0], 8);
...
/*
  ...
  if (tx_int_flag)
  {
    printf("Transmitted first correctly.");
    tx_int_flag = 0;
  }
  */
...
/*
  ...
  if (tx_int_flag)
  {
    printf("Transmitted second correctly.");
    tx_int_flag = 0;
  }
