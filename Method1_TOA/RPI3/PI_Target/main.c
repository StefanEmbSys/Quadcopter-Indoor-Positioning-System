...
int main ()
{
    ...
    int i = 0;
    int receivedChar = 0;
    const char* port = "tcp://192.168.0.27:6050";
...
/*
    ...
    fprintf(stdout, "configRole = %i \r\n", configRole);
    fprintf(stdout, "configEui_low = %i \r\n", (uint32_t)configEui);
    fprintf(stdout, "configEui_high = %i \r\n", (uint32_t)(configEui >> 32));
    ...
    {
        fprintf(stdout, "%i ", (uint8_t)buffer_uart[i]);
...
*/
    ...
    while(1)
    {
        ...
        fprintf (stdout, "Received TOA message: \r\n"); //%c for character (ASCII), %d for decimal value, %i for hexadecimal value
        i = 0;
