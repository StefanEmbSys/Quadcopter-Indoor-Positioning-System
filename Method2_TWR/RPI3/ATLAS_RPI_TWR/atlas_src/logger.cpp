...
Logger::Logger ()
{
    ...
    m_positionLog << "iso,unix_edit,eui,x,y,z,dx,dy,dz,ecef_x,ecef_y,ecef_z,wgs84_lat,wgs84_lon,wgs84_alt,wgs84_heading,wgs84_speed" << std::endl;

    m_sampleLog.open(dir + "_samples.csv");
    m_sampleLog << "iso,unix_edit,txeui,seq,rxeui,toa,ts" << std::endl;
}
...
void Logger::logPosition(const position_t &p)
{
    ...
    double unix_edit = micros / 1000000.0;

    m_positionLog << iso << "," << std::setprecision(6) << std::fixed << unix_edit << ",";
    m_positionLog << std::hex << p.eui << "," << std::dec << std::setprecision(3) << std::fixed << p.pos.at(0) << "," << p.pos.at(1) << "," << p.pos.at(2) << ",";
...
void Logger::logSample(const sample_t &s)
{
    ...
    for (auto it = s.meas.begin(); it != s.meas.end(); ++it)
    {
        ...
        m_sampleLog << iso << "," << std::setprecision(6) << std::fixed << unix_edit << ",";
        m_sampleLog << std::hex << s.txeui << "," << std::dec << s.seq << "," << std::hex << it->second.rxeui << "," << std::setprecision(12)  << std::fixed << it->second.distance << ",";
